...
public abstract class DTOFetcher<DTO_KEY, DTO> {
  ...
  public Map<DTO_KEY, DTO> findWithKeys(Set<DTO_KEY> ids) {
    ...
    Map<DTO_KEY, List<DTO>> keysWithDuplicates = StreamSupport
        .stream(partition(ids, QUERYSTRING_LENGTH_LIMITING_BATCH_SIZE).spliterator(),
            false) //partition into chunks to get data in batches
