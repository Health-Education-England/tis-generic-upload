...
public abstract class DTOFetcher<DTO$KEY, DTO> {
	...
	protected Function<List<DTO$KEY>, List<DTO>> dtoFetchingServiceCall;
	protected Function<DTO, DTO$KEY> keyFunction;
	protected Set<DTO$KEY> duplicateKeys;

	public Map<DTO$KEY, DTO> findWithKeys(Set<DTO$KEY> ids) {
		if(keyFunction == null) {
		...
		Map<DTO$KEY, List<DTO>> keysWithDuplicates = StreamSupport.stream(partition(ids, QUERYSTRING_LENGTH_LIMITING_BATCH_SIZE).spliterator(), false) //partition into chunks to get data in batches
	...
	public Set<DTO$KEY> getDuplicateKeys() {
		return duplicateKeys;
	}

	public Set<Long> extractIds(Map<DTO$KEY, DTO> dtos, Function<DTO, Long> idFunction) {
